...
func generateMarkdown(results []BenchResult, concurrencyResults []ConcurrencyResult, runs int, hasBun bool, section string) string {
	...
	if showAll {
		...
		fmt.Fprintf(sb, "Statistical benchmark results from %d runs per test case.\n\n", runs)
		fmt.Fprintf(sb, "> **Environment:** %s %s (%s/%s)  \n", getCPUModel(), runtime.Version(), runtime.GOOS, runtime.GOARCH)
		fmt.Fprintf(sb, "> **Generated by:** `go run ./cmd/benchmark -runs=%d`  \n", runs)
	...
	if showComparison {
		...
		fmt.Fprintf(sb, "_Statistical results from %d runs per test case (after warmup)._\n\n", runs)

		sb.WriteString("```\n")
		fmt.Fprintf(sb, "%-25s | %-15s | %-15s | %s\n", "Test Case", "GOJA", "Bun", "Winner")
	...
	if showDetailed {
		...
		sb.WriteString("```\n")
		fmt.Fprintf(sb, "%-25s | %-12s | %-12s | %-12s | %-12s | %-12s\n",
